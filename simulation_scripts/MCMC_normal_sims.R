...
sigma1 <- 0.195           # standard deviation for emission densities of all states
means1 <- c(0.55,1,1.9)   # state-specific means for emission densities
...
plot(res_opt_n3, simulation=TRUE,means1,sigma1,true_T1,simdata1full$states)
...
summary(res_n3)
#   graphical diagnostics and confusion matrix
plot(res_n3, simulation=TRUE,pr_means,sd(simdata1)/3,rand_T,simdata1full$states)
...
x <- conf_mat(N=L1,res=res1_n3,plot=TRUE) # SHOULD WORK BUT DOES NOT???? SO...
plot_confusion_matrix(x$`Confusion Matrix`[[1]],add_sums = TRUE)
